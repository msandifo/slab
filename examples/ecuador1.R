...
mex <-assemble_slab(slab.name="sam", extend.lims = c(-5, -5, 23, 1),  simplify=F)
...
ehb.ppp <- project_ppp( mex$ehb.ppp, plates)
...
(p.map<- ggplot( ) +
    ...
    geom_polygon(data=coasts.df   ,
     ...
     geom_path(data=mex$plates %>% fortify()  ,
              aes(x=long, y=lat ,group=id ), size=.4, show.legend = F) +
...
(p.map.a <- p.map + geom_segment(data=cmt.ppp$marks %>% subset(profile !="p0" & bearing >= -10 ),
    ...
    geom_text(data=data.frame(x=c(-101.5,-98.5,-95,-92.5), y=c(16,14.95,13.8, 12.6), profile=c("p1","p2","p3", "p4")),
              aes(x,y, label=profile, col=profile),size=7)+
...
ggsave(paste0(fig.dir,"/ecu.slab.map.pdf"), p.map.a ,width=14, height=9)
...
( p.profile <- ggplot()+
  # geom_point(data=cmt.axes  %>% subset( #profile != "p0"  &
  #                                         depth.anom >= 0  & scalarmoment < 15   ),
  #            aes(boundary.distance, -depth, colour=regime, size=scalarmoment ),
  #            shape=1, stroke=.3, show.legend = F) +
     # geom_point( data=cmt.axes %>% subset( #profile != "p0"   &
     #                                       depth.anom < 0  & scalarmoment < 15   ),
     #          aes(boundary.distance, -depth, colour=regime, size=scalarmoment ),
    ...
    geom_point( data=ehb.ppp$marks %>%subset( #profile != "p0"  &
      depth.anom >= -5),
                ...
                size=.1, show.legend = F, col="grey70") +
...
(p.profile.T <- p.profile +
   #  ggalt::geom_xspline(data=mex.slab.pro, aes(x,y),  size=.35, linetype=5, spline_shape=1)+
    ...
    geom_segment(data=cmt.axes %>% subset(#profile != "p0"   &
                                            axis== "T" &# profile !="pa" &
                                              ( depth.anom> -10 )),
    ...
    geom_point(data=vo.ppp$marks, # %>%  subset(profile != "p0" & profile != "pa"),
               aes(boundary.distance, 1), shape =24,  size=3, col="grey30", fill="Yellow")+
    coord_fixed(ratio = 1, xlim=c(-10, 800), ylim=c(-400,0))+
    labs(title="T-axes")+
    geom_point(data=mex.city_0  %>% subset(#profile !="p0" &
                                             (m>=6. )) %>%  unique(),  aes(boundary.distance, -depth,  size=scalarmoment),size=5, shape=1, colour="white", stroke=1.1,show.legend = F)+
    geom_point(data=mex.city_0   %>% subset(#profile !="p0" &
                                              (m>=6.  ))%>%  unique(),  aes(boundary.distance, -depth, size=scalarmoment),size=5, colour="black",shape=1,stroke=.5, show.legend = F)+
    # ggrepel::geom_text_repel(data=mex.city_0  %>% subset(#profile !="p0" & (m>=6.  ) &
    #                                                        depth <55) %>%  unique()  ,  aes(boundary.distance, -depth, label=paste0((date %>% as.Date) %>% as.character(),"\n",m)),
...
dev.off();
ggsave(paste0(fig.dir,"ec.slabs.dist.T.pdf"), plot=p.profile.T,width=12, height=8)

(p.profile.P <- p.profile +
   # ggalt::geom_xspline(data=mex.slab.pro, aes(x,y),  size=.35, linetype=5, spline_shape=1)+
    #   geom_point(data=mex.slab.pro, aes(x,y))+
    geom_segment(data=cmt.axes %>% subset( #profile != "p0" &
                                             axis== "P" & profile !="pa" &( depth.anom> 0 )), aes(x=x,xend=xend,y= -y, yend= -yend, col=regime), size=.25)+
    # geom_segment(data=ta %>% subset(depth.anom> 0 | depth>40),  aes(x=x,xend=xend,y= -y, yend= -yend, col=regime))+
    geom_point(data=vo.ppp$marks ,#%>% subset( profile != "p0"),
               aes(boundary.distance, 1), shape =24,  size=3, col="grey30", fill="Yellow")+
...
dev.off();
ggsave(paste0(fig.dir,"ecuador.slabs.dist.P.pdf"), plot=p.profile.P,width=12, height=8)
