...
fig.dir="/Users/msandifo/Dropbox/msandifo/documents/programming/r/packages/slab/examples/chiapas/figures"
...
plates.df= mex$plates[mex$plates$ID %in% c("57", "54", "123") ] %>% fortify()
...
# bird.plates.psp =readShapeLines(bird.plates.fn) %>% crop(extent(mex$slab)) %>% as.psp()
message("Bird plate boudary ids : " , mex$plates$ID%>%  unique  %>% str_c( collapse = ", "))
ggthrust( (mex$plates %>% fortify())[,1:2],  h =5, N=barb.n, REV = T, endtol=0.01, scale=barb.scale  )->teeth.s


#cmt.ppp <-cmt.unpruned.ppp <- project_ppp( mex$cmt.ppp, plates, profile="p0")

#cmt.ppp <-cmt.unpruned.ppp$marks<-mex$cmt.ppp
...
mex$cmt.ppp <- mex$cmt.ppp %>% subset( !duplicated(eventname))
 ...
 # p2n <- pwin( mex$cmt.ppp,
 #              win=get_poly(long=mean(-97.3, -100.4), lat=mean(14.67,  15.8), angle=16, width=3.5, length=5 ,owin=F),
 #             n=T)
p2=get_poly(long=-94.6, lat=13.5, angle=30, width=2, length=5 )
l2 =
  get_line(long=-94.6, lat=13.5, angle=30,   length=5,  n=1, psp=T)

p2=get_poly(long=-95., lat=14, angle=30, width=1.6, length=5 )
l2 =
  get_line(long=-95., lat=14, angle=30,   length=5,  n=1, psp=T)
...
#get_isc_srn_rect(rect=mex$slab,year=1960, min.mag=3.5) %>% get_this_isc(csv=T) %>% isc_ppp() -> isc.ppp

project_ppp(isc.ppp[p2], p2) -> isc1.ppp
isc1.ppp[p2a]$marks$distance <- -abs(  isc1.ppp[p2a]$marks$distance)
plot(isc1.ppp[p2]$marks$distance, -isc1.ppp[p2]$marks$depth )
plot(marks$distance, -marks$depth, data= isc1.ppp[p2] )


plot(ras_owin(mex$slab$topo))
points(isc1.ppp[p2] %>% subset( date>=lubridate::ymd("2013-01-01") & m>5), col="red" )
purrr::map(get_ieb_files(), get_ieb) %>% bind_rows() -> ieb3.df



 deps <-extract(mex$slab$depth, l2 %>% as.SpatialLines.psp(), df=F, cellnumbers=T, method="bilinear") %>% as.data.frame()
...
#  geom_path(aes(x=c(-97.5, -99.95, -98.2,-96.3,-97.5) , y=c(15.1, 16, 19.7,19.0,15.1)), linetype=1, size=.25,color="blue3")+
    ...
    annotate("text", x=-95.6,y=14, label="b ", size=4, colour="blue3")+
...
(p.2b <-ggplot()+
    ...
    geom_point(data=ehb1.ppp$marks %>% subset(profile== "p0"   ),
               aes( distance, -depth, size=m), show.legend = T, stroke=.3, alpha=.5, shape=1, fill="white") +
    geom_point(data=cmt1.ppp$marks %>% subset(  profile== "p0"   ),
               aes (distance, -depth, colour=regime, size=m), shape=1, stroke=.3, show.legend = F) +
    geom_point(data=vo1.ppp$marks %>% subset(profile=="p0" ),aes( distance, -depth), shape=24, size=2, fill="yellow", col="red3")+
...
mod1 <-read.csv("/Users/msandifo/Dropbox/msandifo/documents/programming/r/packages/slab/examples/chiapas/slabTop.csv", col.names=c("x", "depth")) %>%
subset(x>= -offset-100)%>%
  mutate(x= x+offset, depth = -depth)

mid1 <-read.csv("/Users/msandifo/Dropbox/msandifo/documents/programming/r/packages/slab/examples/chiapas/midPlane.csv",
                col.names=c("x", "depth")) %>%
  subset(x>= -offset-100)%>%
  mutate(x= x+offset, depth = -depth)

# c650 <-read.csv("/Users/msandifo/Dropbox/msandifo/documents/programming/r/packages/slab/examples/chiapas/650C_contour.csv",
#                 col.names=c("x", "depth")) %>%
#   subset(x>= -offset-100)%>%
#   mutate(x= x+offset, depth = -depth)
...
(p.2c <- p.2b +
  ...
  geom_path(data=depths.ppp$marks %>% subset(!is.na(depth)), aes(distance, depth), col="red3")+
...
curvGrad1 <-read.csv("/Users/msandifo/Dropbox/msandifo/documents/programming/r/packages/slab/examples/chiapas/curvGrad.csv", col.names=c("x", "cg")) %>%
  subset(x>= -offset-200)%>%
  mutate(x= x+offset)
...
(p.kernel.cmt.1<-plot_kernel1(cmt1.ppp$marks %>% subset(profile =="p0"  &
    ...
    geom_line(data=curvGrad1, (aes(x*1, abs(cg*100))), col="blue3", size=.3, linetype=5)


)


(p.kernel.cmt.2<-plot_kernel1(cmt1.ppp$marks %>% subset(profile =="p0"  & (depth.anom >= -0 | is.na(depth.anom) | depth >= 40)),
                             min=5,cuts= c( mm,10), points=T, bw="SJ", kernel="gaussian", mlimit=5,
                             stroke=.5,
                             leg=c(.7,.95), mscale=2.5, xlim=c(-110,400 ), ylim=c(0,0.009), regime=T)+
 #  geom_density( data=ehb1.ppp$marks %>% subset(profile =="p0"  &  m>5.9 &(depth.anom >= -0 | is.na(depth.anom) | depth >= 40)), aes(distance), col="red", linetype=2)+
   ...
   # annotate("text", 280, .0145, label="MFS", size=4.)+
    guides(fill="none")+theme(legend.direction = "horizontal")+
 #   annotate("text", 182, .0145,label="seismic gap", size=4.)+
    guides(fill="none")+theme(legend.direction = "horizontal")+
  #  geom_hline(yintercept= (mm-5)/ (   (1000/4.5)+4), linetype=1, size=.2)+
  #  geom_line(data=curvGrad1, (aes(x  , abs(cg*60))), col="blue3", size=.2, linetype=1)+scale_size(range=c(.5,6))+
  geom_line(data=curvGrad1, (aes(x*mod.fac , abs(cg*100))), col="blue3", size=.4, linetype=5)+scale_size(range=c(.5,6))
...
p.kernel.cmt.3 <-p.kernel.cmt.2+
  geom_point(data= cmt1.ppp$marks %>% subset(profile =="p0"  & depth.anom < 0 & distance<120  ),
             ...
             size=1.5,show.legend=F, alpha=.5,shape=2, stroke=.2)+
  annotate("text",  120, .0135, label="CHIAPAS", size=3, col="blue4")+
...
plot.combo1.b <-cowplot::plot_grid(p.join+
                                   ...
                                   p.2c +
                                     ...
                                     scale_y_continuous(expand = c(0,0),  limits=c(-255,10))+
                                     annotate("text",  90, -78, label="CHAIPAS", size=3, col="blue4")
                                   ,
                                   p.kernel.cmt.3+
                                     theme_update(panel.grid.minor =element_blank(), panel.grid.major =element_line(colour = "grey85", size=.065, linetype=1))+
                                                    guides(fill="none")+
                                     theme(legend.direction = "vertical" , legend.position=c(.1,.7), legend.title = element_blank(),  legend.box = 'horizontal')+
