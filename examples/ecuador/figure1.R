...
########
plot_profile<- function(poly, vo=T, t=T, p=F,slab=T, scale=c(.5,6)){


  plot1 <- ggplot()+
    geom_point(data= poly$ehb[poly$p]$marks %>%  subset(m>4),
               aes(distance, -depth , size=m)    , fill="white", show.legend = F, alpha=1, col="grey30") +
  ...
  if (t)   plot1 <-  plot1 +
     ...
     if (p)     plot1 <- plot1 +
      ...
      # geom_segment(data=ta %>% subset(depth.anom> 0 | depth>40),  aes(x=x,xend=xend,y= -y, yend= -yend, col=regime))+
    if (slab ==T)
    plot1 <- plot1 + geom_line(data=poly$depths$m$marks[  seq(1,length(poly$depths$m$marks$distance), 5),] %>% subset(!is.na(depth)), aes(distance, depth), col="grey60", linetype=1, size=.2)+
      geom_line(data=poly$depths$r$marks[  seq(1,length(poly$depths$r$marks$distance), 5),] %>% subset(!is.na(depth)), aes(distance, depth), col="grey50", linetype=2, size=.2)+
      geom_line(data=poly$depths$l$marks[  seq(1,length(poly$depths$l$marks$distance), 5),] %>% subset(!is.na(depth)), aes(distance, depth), col="grey50", linetype=5, size=.2)
...
(p.join <-  p.sig.mex +
    ...
    geom_path(data= owin_poly(poly3$p), aes(x=x, y=y), linetype=1, size=.25,color="blue3")+
...
profile.p2 <-gridExtra::grid.arrange(#p.join,
                        plot_profile(poly4, t=T, vo=T) +ylim(c(-300,10))+xlim(c(0,630))+theme(legend.position = c(.1,.3))  + annotate("text", x=c(0,630), y=c(10,10), label=c("NW", "SE"), size=6)+ labs(subtitle="a)"),
...
profile.p1 <-gridExtra::grid.arrange(#p.join,
  ...
  plot_profile(poly2, t=T, vo=F)+ylim(c(-300,10))+xlim(c(0,800))+theme(legend.position = "None") + annotate("text", x=c(0,800), y=c(10,10), label=c("W", "E"), size=6)+ labs(subtitle="b)"),
