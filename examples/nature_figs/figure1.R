...
fig.dir="/Users/msandifo/Dropbox/msandifo/documents/programming/r/packages/slab/examples/nature_figs/figures/"
...
#purrr::map(get_ieb_files(), get_ieb) %>% bind_rows() -> ieb3.df
 ...
 deps <-raster::extract(mex$slab$depth, l2 %>% as.SpatialLines.psp(), df=F, cellnumbers=T, method="bilinear") %>% as.data.frame()
...
(p.sig.mex<-ggplot( ) +
    ...
    geom_line(data=mex$borders%>% fortify(), aes(long, lat, group=group), fill="white",show.legend = F, alpha=.4, col="grey90", size=.2)+
