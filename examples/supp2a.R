...
mex <-assemble_slab(slab.name="cam", extend.lims = c(-2, -7, 5.5, .5),  simplify=T, slab2=T)
...
bird.plates <- sf::st_read(bird.plates.fn) %>% st_crop(extent(mex$slab)) %>% as_Spatial() %>% fortify()
#bird.plates.psp =readShapeLines(bird.plates.fn) %>% crop(extent(mex$slab)) %>% as.psp()
bird.plates.psp =sf::st_read(bird.plates.fn) %>% st_crop(extent(mex$slab)) %>% as_Spatial() %>% as.psp()
...
(p.sig.mex<- ggplot( ) +
     ggalt::geom_cartogram( data=coasts.df, aes(map_id=id), map=coasts.df , fill="grey90",alpha=1, size=.1, col="grey50") +

    ggalt::coord_proj(proj.laea, xlim=extent(mex$slab)[1:2],ylim=extent(mex$slab)[3:4]) +
    geom_path(data=mex$borders %>% subset(level==1)%>% fortify(), aes(long, lat, group=group), fill="white",show.legend = F, alpha=.4, col="grey50", size=.2)+
...
(p.kernel.cmt.2<-plot_kernel1(cmt1.ppp$marks %>% subset(profile =="p0"  & (depth.anom >= -0 | is.na(depth.anom) | depth >= 40)),
  ...
  #  geom_hline(yintercept= (mm-5)/ (   (1000/4.5)+4), linetype=1, size=.2)+
    geom_line(data=curvGrad1, (aes(x , abs(cg*100))), col="blue3", size=.5, linetype=5)+scale_size(range=c(.5,6))
...
plot.combo1.b <-cowplot::plot_grid(p.join+
                                   ...
                                   p.kernel.cmt.3+
                                     theme_update(panel.grid.minor =element_blank(), panel.grid.major =element_line(colour = "grey85", size=.065, linetype=1))+
                                                    guides(fill="none")+
                                     theme(legend.direction = "vertical" , legend.position=c(.1,.65), legend.title = element_blank(),  legend.box = 'horizontal')+
