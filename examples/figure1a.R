...
get_isc_srn_rect(year=1960, min.mag=3.5) %>% get_this_isc(csv=T) %>% isc_ppp() -> isc.ppp
...
(p.2ca <- p.2b +
    ...
    labs(x=NULL, y="depth - kms")+
    geom_line(data=mod1 , aes(x,depth ),  size=.35, linetype=1, col="blue3")+
    geom_line(data=mod200 , aes(x,depth+4 ),  size=.75, linetype=4, col="blue3" )+
...
(p.kernel.cmt.2<-plot_kernel(cmt1.ppp$marks %>% subset(profile =="p0"  & (depth.anom >= -0 | is.na(depth.anom) | depth >= 40)),
                             min=5,cuts= c( mm,10), points=T, bw="SJ", kernel="gaussian", mlimit=5, stroke=.5,
                             leg=c(.7,.95), mscale=4.5, xlim=c(-110,400 ), ylim=c(0,0.015), regime=T)+
 #  geom_density( data=ehb1.ppp$marks %>% subset(profile =="p0"  &  m>5.9 &(depth.anom >= -0 | is.na(depth.anom) | depth >= 40)), aes(distance), col="red", linetype=2)+
    ...
    annotate("text", 182, .0145,label="seismic gap", size=4.)+ guides(fill="none")+theme(legend.direction = "horizontal")+
  #  geom_hline(yintercept= (mm-5)/ (   (1000/4.5)+4), linetype=1, size=.2)+
  ...
  geom_line(data=curvGrad1, (aes(x  , abs(cg*160))), col="blue3", size=.4, linetype=1)+
    geom_line(data=curvGrad200, (aes(x  , abs(cg*80))), col="blue3", size=.7, linetype=4)+
...
p.kernel.cmt.3 <-p.kernel.cmt.2+
  ...
  annotate("text",  320, .0105, label="PUEBLA", size=3, col="blue4")+
...
mfs.ehb <-(ehb1.ppp %>% subset(distance > min.dis))$marks


mfs.cmt <-(cmt1.ppp %>% subset(distance > min.dis & !duplicated(eventname)))$marks
...
ggsave(paste0(fig.dir,"/mfs.depths.pdf"),   plot=merge.mfs.depths,width=8, height=9)
